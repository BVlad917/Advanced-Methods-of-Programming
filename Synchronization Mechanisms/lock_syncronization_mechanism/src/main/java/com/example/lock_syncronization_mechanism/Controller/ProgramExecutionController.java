package com.example.lock_syncronization_mechanism.Controller;
...
import com.example.lock_syncronization_mechanism.Model.ADT.*;
...
import com.example.lock_syncronization_mechanism.Model.Statement.IStatement;
import com.example.lock_syncronization_mechanism.Repository.IRepository;
import com.example.lock_syncronization_mechanism.Repository.Repository;
...
public class ProgramExecutionController {
    ...
    @FXML
    private TableColumn<Pair<Integer, Integer>, Integer> lockTableLockIDColumn;

    @FXML
    private TableColumn<Pair<Integer, Integer>, Integer> lockTableThreadIDColumn;
    ...
    void initialize() {
        ...
        this.lockTableLockIDColumn.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().getKey()).asObject());
        this.lockTableThreadIDColumn.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().getValue()).asObject());
    ...
    public void createProgramState(IStatement program, String repositoryFilePath) {
        ProgramState initialProgramState = new ProgramState(new MyStack<>(), new MyDictionary<>(), new MyList<>(), new MyFileTable<>(), new MyHeapTable<>(), new LockTable(), program);
    ...
    private void populateLockTable() {
        ArrayList<Pair<Integer, Integer>> lockTableElems = new ArrayList<>();
        this.currentProgramState.getLockTable().getContent().forEach((key, value) -> lockTableElems.add(new Pair<>(key, value)));
        this.lockTable.setItems(FXCollections.observableList(lockTableElems));
    }
