package com.example.countdown_latch_synchronization_mechanism.Controller;
...
import com.example.countdown_latch_synchronization_mechanism.Model.Statement.IStatement;
...
public class ProgramExecutionController {
    ...
    @FXML
    private TableColumn<Pair<Integer, Integer>, Integer> latchTableCounterColumn;
    ...
    void initialize() {
        ...
        this.latchTableCounterColumn.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().getValue()).asObject());
    ...
    public void createProgramState(IStatement program, String repositoryFilePath) {
        ProgramState initialProgramState = new ProgramState(new MyStack<>(), new MyDictionary<>(), new MyList<>(), new MyFileTable<>(), new MyHeapTable<>(), new MyLatchTable(), program);
    ...
    private void populateLatchTable() {
        ArrayList<Pair<Integer, Integer>> latchTableElems = new ArrayList<>();
        this.currentProgramState.getLatchTable().getContent().forEach((key, value) -> latchTableElems.add(new Pair<>(key, value)));
        this.latchTable.setItems(FXCollections.observableList(latchTableElems));
    }
